...
static bool inspect_float(const char* title, float& flt, vec2 range = vec2{0.0f})
{
	ImGui::PushID(title);
...
static bool inspect_vec3(const char* title, vec3& vector, bool is_unit = false)
{
	ImGui::PushID(title);
...
static bool inspect_quat(const char* title, Quaternion& quat)
{
	ImGui::PushID(title);
...
static bool inspect_transform(Transform& transform)
{
...
static void inspect_camera(camera_component& camera)
{
...
static void inspect_directional_light(directional_light_component& light)
{
...
static bool inspect_point_light(point_light_component& light)
{
	bool dirty = false;
...
static bool inspect_spot_light(spotlight_component& light)
{
	bool dirty = false;
...
static void inspect_material(const ResourceID& rid)
{
	...
	if(ImGui::CollapsingHeader("Material"))
	{
		...
		if(mtl.flags & MATERIAL_ALPHA_MASK)
		{
			has_alpha_cutoff = true;
			ImGui::Text("MATERIAL_FLAG_ALPHA_MASK");
		}
		else if(mtl.flags & MATERIAL_ALPHA_BLEND)
		{
		...
		if(mtl.flags & MATERIAL_CLEARCOAT)
		{
...
static void inspect_animation(animation_component& anim)
{
	if(ImGui::CollapsingHeader("Animation"))
	{
		...
		if(ImGui::BeginDragDropTarget())
		{
			if(const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("resource_payload"))
			{
				...
				if(resource_get_type(rid) == RESOURCE_TYPE_ANIMATION && resource_get_handle(rid))
				{
					anim.animation = rid;
				}
			}
		...
		{
			anim.animation = resource_manager_load_animation(vfs_path{"anim"} / anim_name);
		}
...
static void inspect_rigidbody(rigidbody_component& rb)
{
...
static void inspect_sphere_collider(sphere_collider_component& scol)
{
	if(ImGui::CollapsingHeader("Sphere collider"))
	{
		inspect_float("Radius", scol.desc.radius);
	}
}

static void inspect_capsule_collider(capsule_collider_component& ccol)
{
...
static void inspect_box_collider(box_collider_component& bcol)
{
	if(ImGui::CollapsingHeader("Box collider"))
	{
		inspect_vec3("Edges", bcol.half_size);
	}
}	

Inspector::Inspector(WorldState* ws) : Widget("Inspector"), world{ws} {}
...
void Inspector::on_draw()
{
	...
	ImGui::InputText("##ent_name", &name, tflags);
	ImGui::SameLine();
...
//		auto* emitter = graph.try_get<emitter_component>(world->selected_entity);
	...
	auto* scol = graph.try_get<sphere_collider_component>(world->selected_entity);
	auto* ccol = graph.try_get<capsule_collider_component>(world->selected_entity);
	auto* bcol = graph.try_get<box_collider_component>(world->selected_entity);

	bool has_collider = scol || ccol || bcol;
...
*/
		...
		{
			auto body = physics_create_body({});
			rb = &graph.emplace<rigidbody_component>(world->selected_entity, physicsBodyDesc{}, body);
		}
		...
		{
			bcol = &graph.emplace<box_collider_component>(world->selected_entity, vec3{0.5f});
		}
	...
	if(scol)
		inspect_sphere_collider(*scol);

	if(ccol)
		inspect_capsule_collider(*ccol);

	if(bcol)
		inspect_box_collider(*bcol);
}
