...
constexpr static std::array<const char*, 1> default_instance_extensions =
{
...
constexpr static size_t max_bindless_textures = 65536;
constexpr static size_t max_bindless_samplers = 32;
...
static std::vector<VkDeviceQueueCreateInfo> vulkan_device_create_queues()
{
	u32 qf_count{0};
...
static void vulkan_device_setup_queues()
{
...
static bool vulkan_create_device(std::span<VkPhysicalDevice> phys_devices, int index = -1)
{
	bool found = false;
	u32 max_score = 0;
	...
	const VkDeviceCreateInfo device_ci
	{
		...
		.pNext = &chain_devf2,
		.queueCreateInfoCount = static_cast<u32>(queue_ci.size()),
	...
	std::string ext_msg;
	for(u32 i = 0; i < device_ci.enabledExtensionCount; i++)
		ext_msg += std::format("\n\t{}", device_ci.ppEnabledExtensionNames[i]);

	log::info("gpu_vulkan: device extensions: {}", ext_msg);
...
static bool vulkan_context_init()
{
	...
	log::info("gpu_vulkan: instance extensions: {}", ext_msg);
...
static void vulkan_setup_descriptor_heaps()
{
...
static void gpu_cleanup_swapchain();
...
static std::optional<u32> get_memory_type(u32 type, VkMemoryPropertyFlags flags)
{
	...
	for(u32 i = 0; i < mp.memoryProperties.memoryTypeCount; i++)
	{
		if((type & 1) == 1)
		{
...
u8* gpu_map_memory(const GPUPointer& ptr)
{
	...
	return std::bit_cast<u8*>(buffer.mapped) + ptr.offset;
}
...
static VkImageView get_or_create_image_view(GPUTextureData& tex, const GPUViewDesc& desc)
{
...
u64 gpu_semaphore_read_counter(GPUSemaphore sem)
{
	assert(sem);
...
static VkDescriptorSetLayout shader_create_push_descriptor_set(const ShaderIR::CBufferInfo& info)
{
	const VkDescriptorSetLayoutBinding binding
	{
		...
		.descriptorCount = 1u,
		.stageFlags = shader_stage_to_vk_flags(GPUStage{info.stage_flags}),
		.pImmutableSamplers = nullptr
	};
...
static std::pair<VkPipelineLayout, u64> shader_create_pipeline_layout(const ShaderIR& shader)
{	
	...
	if(shader.cbuffer.size)
	{
		...
		pdsl_handle = std::bit_cast<u64>(ds_layouts[0]);
	}
	else
	{
	...
	{
		.stageFlags = shader_stage_to_vk_flags(GPUStage{shader.pconst.stage_flags}),
		.offset = 0,
...
GPUPipeline gpu_create_compute_pipeline(const ShaderIR& shader)
{
	...
	const VkShaderModuleCreateInfo sm_info
	{
		...
		.pNext = nullptr,
		.flags = 0,
		.codeSize = shader.stages[0].spirv.size() * sizeof(u32),
	...
	return
	{
		std::bit_cast<u64>(pipe),
		std::bit_cast<u64>(layout),
		pdsl_handle,
		static_cast<u32>(shader.cbuffer.size),
		static_cast<u32>(shader_stage_to_vk_flags(GPUStage{shader.pconst.stage_flags})),
		static_cast<u32>(shader.pconst.size),
		true
	};
}
...
GPUPipeline gpu_create_graphics_pipeline(const ShaderIR& shader, const GPURasterDesc& raster)
{
	...
	for(auto& stage : shader.stages)
	{
		...
		sm_info[num_stages] =
		{
			...
			.pNext = nullptr,
			.flags = 0,
			.codeSize = stage.spirv.size() * sizeof(u32),
		...
		stages[num_stages] = 
		{
			...
			.flags = 0,
			.stage = shader_stage_to_vk(GPUStage{stage.pipeline_stage}),
	...
	for(u32 i = 0; i < raster.color_targets.size(); i++)
	{
	...
	const VkPipelineColorBlendStateCreateInfo blend
	{
		...
		.flags = 0,
		.logicOpEnable = false,
		.attachmentCount = static_cast<u32>(raster.color_targets.size()),
...
void gpu_copy_to_texture(const GPUCommandBuffer& cmd, const GPUPointer& src, GPUTexture dst, u32 mips, u32 layers)
{
	assert(src.handle);
...
{
	assert(!cmd.wait_signal.object && "Command buffer already waiting on signal");
	cmd.wait_signal =
	{
		sem,
		timeline,
...
{
	assert(sem);
	assert(!cmd.emit_signal.object && "Command buffer already emitting signal");
	cmd.emit_signal =
	{
		sem,
		timeline, 
...
void gpu_begin_renderpass(const GPUCommandBuffer& cmd, const GPURenderPassDesc& rp)
{
	...
	{
		{
		static_cast<s32>(rp.render_area.x),
		static_cast<s32>(rp.render_area.y)
		},
...
void gpu_set_scissor(const GPUCommandBuffer& cmd, uvec4 scissor)
{
	...
	{
		{
		static_cast<s32>(scissor.x),
		static_cast<s32>(scissor.y)
		},
		{scissor.z, scissor.w}
	};
...
void gpu_draw_indexed_indirect_count(const GPUCommandBuffer& cmd, void* data, const GPUPointer& commands, const GPUPointer& draw_count, u32 max_draw_count)
{
	...
	assert(draw_count.offset + sizeof(u32) <= draw_count_buffer.size);
...
static VkExtent2D find_swapchain_extent(const VkSurfaceCapabilitiesKHR& caps)
{
...
static void gpu_create_swapchain()
{
	...
	for(auto img : swapchain_images)
	{
		...
		gpu_context->swapchain_textures.push_back(GPUTexture{static_cast<u32>(gpu_context->textures.size() - 1)});
	}
}
...
{
	if(!SDL_Vulkan_CreateSurface(reinterpret_cast<SDL_Window*>(wnd), gpu_context->instance, nullptr, &gpu_context->swapchain_surface))
	{
